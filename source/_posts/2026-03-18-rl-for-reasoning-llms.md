...
在标准 RL 设置中，智能体观察状态 $s_t$，根据策略 $\pi(a_t \mid s_t)$ 选择动作 $a_t$，根据环境动态 $p(s_{t+1} \mid s_t, a_t)$ 转移到新状态 $s_{t+1}$，并接收奖励 $r_t$。

具体例子是机器人在房间中导航：状态是当前位置和传感器读数，动作是移动命令，转移动态由物理规律控制（轮子可能打滑），奖励反映向目标的进展。

这个循环重复 $T$ 个时间步。智能体目标是最大化期望折扣回报：

$$
J = \mathbb{E}\left[\sum_{t=0}^{T} \gamma^t r_t\right]
$$

其中折扣因子 $0 \leq \gamma \leq 1$ 控制未来奖励的折扣程度。

策略通常由 $\theta$ 参数化。许多 RL 算法的核心对象是价值函数：

$$
V^\pi(s) = \mathbb{E}_\pi\left[\sum_{l=0}^{T-t} \gamma^l r_{t+l} \mid s_t = s\right],
$$

它衡量在策略 $\pi$ 下状态 $s$ 的好坏。由此可以推导出优势（advantages），估计某个动作比预期更好还是更差。

对于 LLM，设置通常大幅简化。我们有一个参数化模型 $\pi_\theta$ 对提示词 $x \sim \mathcal{D}$ 采样响应 $y \sim \pi_\theta(\cdot|x)$，我们用标量奖励 $r(x, y)$ 评分。目标变为：
...
这个比较揭示了 REINFORCE 本质上是加权形式的 SFT。不是强化提供的离策略答案 $y^*$，而是根据奖励加权强化或惩罚采样的在策略答案 $y$。

REINFORCE 的主要缺点是方差。即使奖励相对结构化（例如大型测试套件，每个测试贡献部分奖励），梯度估计在样本间也可能大幅变化。

为了减少方差，REINFORCE 减去一个不依赖于采样动作（响应）的基线 $b(x)$。这使期望梯度不变，因为：

$$
\mathbb{E}_{y \sim \pi_\theta(\cdot \mid x)}\left[\nabla_\theta \log \pi_\theta(y \mid x)\, b(x)\right] = 0,
$$

同时通常大幅减少方差。梯度变为：

$$
\nabla_\theta J(\theta) = \mathbb{E}_{y \sim \pi_\theta(\cdot \mid x)}\left[\nabla_\theta \log \pi_\theta(y \mid x)\, \bigl(r(x,y)-b(x)\bigr)\right].
$$

量 $r(x,y)-b(x)$ 是优势估计的最简单形式。
...
$$
J^{\text{PPO}}(\theta) = \mathbb{E}_t\left[\min\left(\rho_t(\theta)\hat{A}_t,\; \operatorname{clip}(\rho_t(\theta), 1-\epsilon, 1+\epsilon)\hat{A}_t\right)\right],
$$
...
是当前策略 $\pi_\theta$ 与生成轨迹的策略 $\pi_{\theta_\text{old}}$ 之间的重要性采样（IS）比率。
...
注意裁剪不仅影响目标值，更重要的是影响其对 $\theta$ 的依赖。由于我们优化 $\theta$ 以最大化 $J$，裁剪情况产生零梯度，因为学习的策略不再参与方程。这些情况的更新被跳过，因为已移出信任区域。

裁剪处理四种场景：

| | 未裁剪 | 裁剪 |
|---|---|---|
| **正优势**（好答案，强化） | $\rho_t(\theta)\hat{A}_t$：答案好且未过度更新 | $(1+\epsilon)\hat{A}_t$：答案已足够可能，梯度停止 |
| **负优势**（坏答案，抑制） | $\rho_t(\theta)\hat{A}_t$：答案坏且未过度更新 | $(1-\epsilon)\hat{A}_t$：答案已足够不可能，梯度停止 |
...
计算 $\delta$ 需要学习价值函数。在 LLM 设置中，这通常需要额外的价值模型，通常与策略模型大小相当。这在内存上成本高且增加训练复杂性。我们不会在本文中详细检查 GAE，因为移除此组件是 GRPO 的主要实际贡献。有关 PPO 及其所有组件的深入解释，请参阅这篇[详细文章](https://huggingface.co/blog/NormalUhr/rlhf-pipeline)。

最后，PPO 目标通常与 KL 正则化结合：

$$
J^{\text{PPO-KL}}(\theta) = \mathbb{E}_t\left[M(\hat{A}_t,\rho_t,\epsilon)\rho_t(\theta)\hat{A}_t\right] - \beta\, D_{\text{KL}}(\pi_\theta \,\|\, \pi_{\text{ref}}).
$$

这里 $\pi_{\text{ref}}$ 通常是 RL 训练前的模型。在 RLHF 中这项特别重要，因为它保留通用能力并帮助控制相对于奖励模型的分布偏移（奖励模型是在参考策略 $\pi_{\text{ref}}$ 上训练的）。在推理 RL 中，KL 惩罚通常设置得小得多或完全省略 [3][7]。
...
关键见解是可以通过将每个响应与同一提示词的其他响应比较来获得良好的基线。对于每个提示词 $x \sim \mathcal{D}$，GRPO 采样一组 $G$ 个响应 $\{y_1, \ldots, y_G\}$，计算奖励 $r_i = r(x, y_i)$，并在组内归一化奖励以获得优势：
...
$$
J^{\text{GRPO}}(\theta) = \mathbb{E}_{x \sim \mathcal{D}}\left[\frac{1}{G}\sum_{i=1}^{G} \min\left(\rho_i(\theta) \hat{A}_i,\, \text{clip}(\rho_i(\theta), 1-\epsilon, 1+\epsilon) \hat{A}_i\right) - \beta \cdot D_{\text{KL}}(\pi_\theta(\cdot|x) \| \pi_{\text{ref}}(\cdot|x))\right]
$$

其中 $\rho_i(\theta)=\frac{\pi_\theta(y_i \mid x)}{\pi_{\theta_{\text{old}}}(y_i \mid x)}$。

组归一化有两个有用效果。减去均值使学习信号相对于提示词：如果该提示词的所有样本在 $[0.8, 1.0]$ 中，奖励 $0.8$ 应该与在 $[0.2, 0.8]$ 中时不同地解释。除以标准差使奖励尺度不太敏感，这在组合不同奖励范围的任务时有用。
...
对于每个提示词，RLOO 采样 $K$ 个响应 $\{y_1, \ldots, y_K\}$。响应 $y_i$ 的优势是其奖励减去其他 $K-1$ 个响应的平均奖励：
...
第二项改进针对裁剪机制：PPO 的对称比率裁剪对低概率 token 特别（过度）限制。例如，如果 token 概率为 $0.01$，那么用 $\epsilon = 0.2$ 其概率在裁剪前只能上升到 $0.012$，几乎不改变其被采样的可能性。这可能抑制学习稀有但有用的推理延续。因此 DAPO 解耦裁剪边界并使用更大的上界 $\epsilon_{\text{high}} = 0.28$，保持 $\epsilon_{\text{low}} = 0.2$（非对称裁剪）。
...
\operatorname{clip}(\rho_{i,t}(\theta),1-\epsilon_{\text{low}},1+\epsilon_{\text{high}})\hat{A}_i
\right)
\right].
$$
...
其中 $\operatorname{sg}(\cdot)$ 表示停止梯度。

有趣的是，他们报告只需要上裁剪 $\epsilon_h$ 并调优，下裁剪 $\epsilon_l$ 设置得足够高以有效不活跃。
...
MaxRL（Maximum Likelihood Reinforcement Learning）[12] 从不同角度开始：标准 RL 目标优化期望奖励（pass@1），这经常被观察到（pass@1 以 pass@$k$ 为代价提高），但不一定是最合适的目标。相比之下，最大似然训练（如预训练和 SFT 中使用）将最大化 $\log p_\theta(x)$。
...
所以最大似然梯度是 pass@$k$ 梯度的无限调和混合，不仅仅是 pass@1。而标准 RL 只保留该展开的第一阶项。
...
其中 $T=1$ 恢复标准 RL，$T\to \infty$ 恢复最大似然。

期望梯度与此目标匹配的在策略估计器非常简单：给定提示词的 $N$ 个轨迹，让 $K$ 为成功轨迹数。然后 MaxRL 仅平均成功轨迹的得分函数：

$$
\hat{g}_N(x) =
\begin{cases}
\displaystyle \frac{1}{K}\sum_{i=1}^{N} r_i \nabla_\theta \log \pi_\theta(y_i \mid x), & K \ge 1, \\[0.8em]
0, & K = 0.
\end{cases}
$$

这个估计器对 $T=N$ 的截断 MaxRL 目标是无偏的。与 REINFORCE 的关键区别是在这种情况下增加轨迹减少估计器方差，同时也使优化的目标本身更好地近似最大似然。

也可以用带零均值控制变量的 REINFORCE 形式重写估计器，这使加权更明确。如果 $\hat r = K/N$ 是该提示词的成功率，则有效优势变为正比于：

$$
\hat{A}_i^{\text{MaxRL}} \propto \frac{r_i - \hat r}{\hat r}.
$$

这显示了为什么 MaxRL 将学习信号集中在困难提示词上。当 $\hat r$ 小但非零时，该提示词的成功轨迹被强加权。相比之下，$\hat r \approx 1$ 的简单提示词接收相对较少的额外强调。

经验上，MaxRL 改进 pass@$k$，比 GRPO 更好地保留输出多样性，并在测试时缩放效率上产生实质性收益。
...
J^{\text{DPPO}}(\theta) =
\mathbb{E}\left[
M_{\text{div}}\!\left(\widehat{D}(\pi_\theta,\pi_{\theta_{\text{old}}}), \tau\right)\,
\rho(\theta)\,
\hat{A}
\right],
$$

其中 $M_{\text{div}}$ 掩码其估计发散超过阈值 $\tau$ 的更新。
...
| PPO | 带 critic 的 GAE | 对称 IS | $M_\text{sym}(\hat{A}_t, \rho_t, 0.2)$ | 样本平均 | 稳定，更样本高效 |
| GRPO | $(r-\mu_G)/\sigma_G$ | 对称 IS | $M_\text{sym}(\hat{A}_t, \rho_t, 0.2)$ | 长度归一化$^\dagger$ | 更少内存密集 |
| RLOO | 留一平均 | 无 | 无 | 样本平均 | 无 critic 的方差减少 |
| Dr. GRPO | $r - \mu_G$ | 对称 IS | $M_\text{sym}(\hat{A}_t, \rho_t, 0.2)$ | Token 平均$^\ddagger$ | 移除长度偏差和 std 加权 |
| DAPO | $(r-\mu_G)/\sigma_G$ | 非对称 IS | $M_\text{asym}(\hat{A}_t, \rho_t, 0.2, 0.28)$ | Token 平均 | 给小概率更多增长空间 |
| CISPO | 组内 $(r-\mu_G)/\sigma_G$ | 上界 IS | 无 | Token 平均 | 不掩码梯度，仅裁剪 |
| DPPO | 组内 $(r-\mu)/\sigma$ | 对称 DV | $M_\text{div}(\hat{A}_t, \upsilon_t, 0.15)$ | 样本平均 | 使用 DV 信任区域适应 LLM 域 |
| MaxRL | $(r_i - \hat{r})/(N\cdot \hat{r})$ 且 $\hat{r}=K/N$ | 无 | 无 | 样本平均 | RL 和 MLE 间插值，更好 pass@k |
| ScaleRL | $(r-\mu_B)/\sigma_B$ | 上界 IS | 无 | 提示词平均 | 大规模验证和缩放定律 |

$^\dagger$ 实现可能不同，例如 [Huggingface TRL](https://huggingface.co/docs/trl/grpo_trainer#computing-the-loss)
...
$$
\begin{align*}
\rho_t(\theta) &= \frac{\pi_\theta(a|s)}{\pi_{\theta_{\text{old}}}(a|s)} \\
\upsilon_t(\theta) &= \pi_\theta(a|s) - \pi_{\theta_{\text{old}}}(a|s) \\

M_\text{sym}(\hat{A}_t, \rho_t, \epsilon) &= \begin{cases}
0 & \text{if } (\hat{A}_t > 0 \land \rho_t > 1 + \epsilon) \lor (\hat{A}_t < 0 \land \rho_t < 1 - \epsilon) \\
1 & \text{otherwise}
\end{cases} \\

M_\text{asym}(\hat{A}_t, \rho_t, \epsilon_l, \epsilon_h) &= \begin{cases}
0 & \text{if } (\hat{A}_t > 0 \land \rho_t > 1 + \epsilon_h) \lor (\hat{A}_t < 0 \land \rho_t < 1 - \epsilon_l) \\
1 & \text{otherwise}
\end{cases} \\

M_\text{div}(\hat{A}_t, \upsilon_t, \delta) &= \begin{cases}
0 & \text{if } (\hat{A}_t > 0 \land \upsilon_t > \delta) \lor (\hat{A}_t < 0 \land \upsilon_t < \delta) \\
1 & \text{otherwise}
\end{cases}
...
**标准差归一化倾向于有害**。Dr. GRPO 和 MaxRL 都显示除以 $\sigma$ 在几乎解决的问题上增加太多权重。ScaleRL 消融确认 DAPO（带标准差归一化）达到显著更低的渐近性能，相比于 CISPO 和 GSPO [15]（本文未涵盖，可能在扩展中）。

**损失聚合不是小细节**。Dr. GRPO 和 DAPO 显示序列级奖励与样本级平均结合可能扭曲每 token 学习信号。损失的减少是方法的关键部分，错误选择可能引入微妙偏差。

**信任区域是优化的好点**。PPO 的信任区域定义（$\epsilon = 0.2$）似乎非常精心选择，因为它跨模型和任务工作良好。然而，最近许多方法针对信任区域并显示改进性能：DAPO 非对称放松，CISPO 裁剪权重而不是掩码梯度，DPPO 认为采样 token 比率是首先约束的错误量。领域尚未收敛到信任区域的良好定义，可能没有单一、任务和模型无关的定义，但这里的一些进一步研究可能导致持续改进。
